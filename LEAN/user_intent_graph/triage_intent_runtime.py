...
- confirm an entry node before making it current;
- keep the complete ordered yes-history and rejected nodes;
- condition the confirmed entry's fixed path priors on every yes/no answer;
...
- keep the parent current after negative feedback and ask its next sibling;
...
from user_intent_probability import DynamicProbabilityCalculator


DEFAULT_GRAPH_PATH = Path("pipeline_out/head_discomfort_toy/event_graph/graph.json")
DEFAULT_TRACE_LOG = Path("pipeline_out/head_discomfort_toy/intent_trace.jsonl")
...
class TriageIntentRuntime:
    def __init__(
        self,
        graph: ProtocolGraph,
        probability_calculator: DynamicProbabilityCalculator | None = None,
    ) -> None:
        self.graph = graph
        self.probability_calculator = probability_calculator or DynamicProbabilityCalculator(
            {node_id: node.event_type for node_id, node in graph.nodes.items()},
            [(edge.src, edge.dst) for edge in graph.edges],
            node_labels={node_id: node.label for node_id, node in graph.nodes.items()},
        )

    @classmethod
    def from_graph_path(cls, graph_path: Path = DEFAULT_GRAPH_PATH) -> "TriageIntentRuntime":
        return cls(
            ProtocolGraph.from_json(graph_path),
            DynamicProbabilityCalculator.from_graph_path(graph_path),
        )
    ...
    def process_turn(self, state: SessionState, user_text: str) -> dict[str, Any]:
        ...
        probability = self._probability_record(state)
        if probability.get("contradiction") and state.entry_node_id and not state.stop_message:
            state.stop_message = "contradictory evidence: no complete graph path matches all yes/no answers"

        decision, record = self._decision_record(state)
        question = None if decision or state.stop_message else forced_question or self._next_question(state, probability)
        ...
        state.turn_index += 1
        recoverable_notice = state.stop_message == "starting node rejected; provide another graph node"
        state.done = bool(decision or (state.stop_message and not recoverable_notice))
        return self._result(
            user_text=user_text,
    ...
    def _handle_input_match(
        ...
        node_id, _score = matches[0]
        if self._status(state, node_id) not in {"yes", "no"}:
        ...
        elif node_id != state.current_node_id:
            state.pending_question = PendingQuestion(
                node_id=node_id,
                question=self._question_for_node(node_id),
                reason="confirm_matched_node",
                context_node=state.current_node_id,
            )
            state.asked_edges.add((state.current_node_id, node_id))
        return [node_id]
    ...
    def _apply_feedback(
        ...
        if answer == "no":
            ...
            if pending.reason.startswith("confirm_starting"):
                state.entry_node_id = None
                state.current_node_id = None
                state.path = []
                state.stop_message = "starting node rejected; provide another graph node"
            state.pending_question = None
            return current_ids

        self._set_fact(state, node_id, "yes", "answer", user_text, explanation)
        if pending.reason.startswith("confirm_starting"):
            state.entry_node_id = node_id
    ...
    def _next_question(
        ...
        probability_by_id = {
            event["id"]: event.get("p")
            for event in probability.get("events", [])
        }
        for node_id in self.graph.neighbors(current_id):
            ...
            if self._status(state, node_id) in {"yes", "no"}:
                continue
            node_probability = probability_by_id.get(node_id)
            if node_probability is None or node_probability <= 0:
    ...
    def _match_nodes(self, user_text: str) -> list[tuple[str, float]]:
        ...
        for node in self.graph.nodes.values():
            primary_text = normalize_text(f"{node.id} {node.label}")
            secondary_text = normalize_text(f"{node.description} {node.source_text}")
            primary_tokens = set(tokenize_for_match(primary_text))
            secondary_tokens = set(tokenize_for_match(secondary_text))
            normalized_primary_id = normalize_identifier(f"{node.id} {node.label}")

            score = 0.0
            if normalized_query_id and normalized_query_id in normalized_primary_id:
                score = max(score, 3.0)
            if query and query in primary_text:
                score = max(score, 2.9)
            primary_overlap = query_tokens & primary_tokens
            if primary_overlap and query_tokens:
                score = max(score, 2.0 * len(primary_overlap) / len(query_tokens))
            if query and query in secondary_text:
                score = max(score, 1.0)
            secondary_overlap = query_tokens & secondary_tokens
            if secondary_overlap and query_tokens:
                score = max(score, 0.5 * len(secondary_overlap) / len(query_tokens))
            if score > 0:
    ...
    def _result(
        ...
        *,
        ...
        result = {
            ...
            "entryNode": self._node_record(state.entry_node_id) if state.entry_node_id else None,
        ...
        result["interactionPrompt"] = visible_response(result)
        feedback = self._probability_feedback(probability)
        result["visibleResponse"] = "\n".join([*feedback, result["interactionPrompt"]])
        return result

    def _probability_feedback(self, probability: dict[str, Any]) -> list[str]:
        if probability.get("entry") is None:
            return []
        given = probability.get("given", {})
        yes_ids = given.get("yes", [])
        no_ids = given.get("no", [])

        conditions = [
            self.graph.nodes[node_id].label
            for node_id in yes_ids
            if node_id in self.graph.nodes
        ]
        conditions.extend(
            f"not {self.graph.nodes[node_id].label}"
            for node_id in no_ids
            if node_id in self.graph.nodes
        )
        return [
            f"Given {'; '.join(conditions)}:",
            "Probability:",
            *self.probability_calculator.format_lines(probability),
        ]

    def _probability_record(self, state: SessionState) -> dict[str, Any]:
        verified = list(state.path)
        for node_id, fact in state.facts.items():
            if fact.status == "yes" and node_id not in verified:
                verified.append(node_id)
        rejected = [node_id for node_id, fact in state.facts.items() if fact.status == "no"]
        return self.probability_calculator.calculate(
            state.entry_node_id,
            confirmed_yes=verified,
            confirmed_no=rejected,
        )
...
def append_trace_log(path: Path, result: dict[str, Any]) -> None:
    record = {
        ...
        "input": result["input"],
        "visibleResponse": result["interactionPrompt"],
