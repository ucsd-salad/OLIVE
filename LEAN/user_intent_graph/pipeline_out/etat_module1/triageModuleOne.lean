...
def LethargicOrUnconscious (p : Patient) : Prop :=
  p.avpu = AVPU.Verbal ∨
  p.avpu = AVPU.Pain ∨
  p.avpu = AVPU.Unresponsive

instance (p : Patient) : Decidable (LethargicOrUnconscious p) := by
  unfold LethargicOrUnconscious
  infer_instance
...
def DehydrationSignCount (p : Patient) : Nat :=
  (if LethargicOrUnconscious p then 1 else 0) +
  (if p.sunkenEyes then 1 else 0) +
  (if p.skinTurgor = SpeedMeasurement.slow then 1 else 0)

/-- D in ABCD, an indication for emergency triage-/
def Dehydration (p : Patient) : Prop :=
  DehydrationSignCount p ≥ 2
...
def FluidLossConcern (p : Patient) : Prop :=
  p.diarrhea ∨ p.vomiting ∨ p.drinkingPoorly ∨ p.repeatedVomiting

instance (p : Patient) : Decidable (FluidLossConcern p) := by
  unfold FluidLossConcern
  infer_instance

def ModerateDehydration (p : Patient) : Prop :=
  FluidLossConcern p ∧
  ¬ Dehydration p ∧
  (DehydrationSignCount p = 1 ∨ p.drinkingPoorly ∨ p.repeatedVomiting)

instance (p : Patient) : Decidable (ModerateDehydration p) := by
  unfold ModerateDehydration FluidLossConcern Dehydration DehydrationSignCount
  infer_instance
...
def Priority (p : Patient) : Prop :=
  ...
  -- Moderate dehydration or fluid loss concern
  ModerateDehydration p ∨
